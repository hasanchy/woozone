...
//use WooZoneVendor\GuzzleHttp\json_encode;
//use WooZoneVendor\GuzzleHttp\Psr7\build_query;
use WooZoneVendor\GuzzleHttp\ClientInterface;
use WooZoneVendor\GuzzleHttp\Exception\RequestException;
use WooZoneVendor\GuzzleHttp\Psr7\MultipartStream;
use WooZoneVendor\GuzzleHttp\Psr7\Request;
use WooZoneVendor\GuzzleHttp\RequestOptions;
    ...
    protected function getBrowseNodesRequest($getBrowseNodesRequest)
    {
        ...
        if (isset($_tempBody)) {
            ...
            if ($httpBody instanceof \stdClass && $headers['Content-Type'] === 'application/json') {
                $httpBody = \WooZoneVendor\GuzzleHttp\json_encode($httpBody);
            }
            ...
            if ($multipart) {
                ...
                $httpBody = \WooZoneVendor\GuzzleHttp\json_encode($formParams);

            } else {
                // for HTTP post (form)
                $httpBody = \WooZoneVendor\GuzzleHttp\Psr7\build_query($formParams);
            }
        }
        ...
        $query = \WooZoneVendor\GuzzleHttp\Psr7\build_query($queryParams);
        return new Request(
            'POST',
