...
if (class_exists('WooZoneDirectImport') != true) { class WooZoneDirectImport {
	...
	public function register_rest_routes() {
		register_rest_route( 'woozone/v1', '/direct-import/(?P<action>[a-zA-Z0-9_-]+)', array(
			'methods' => 'POST',
			'callback' => array( $this, 'rest_request' ),
	...
	public function rest_permission_check( $request ) {
		...
		if( !current_user_can('administrator') ){
			//return new WP_Error( 'rest_forbidden', __( "Only administrator can access this endpoint!", "WooZone" ), array( 'status' => 403 ) );
		}

		// Validate access key
		$accesskey = $request->get_param('accesskey');
		$opValidateAccess = $this->validate_accesskey( $accesskey );
		
		if ( 'invalid' === $opValidateAccess['status'] ) {
			return new WP_Error( 'rest_invalid_key', $opValidateAccess['msg'], array( 'status' => 401 ) );
		}

		return true;
	}

	public function rest_request( $request ) {
		...
		switch ( $action ) {
			...
			case 'add_product':
				$opArgs = array(
					'avi_nbvars' 	=> $request->get_param('avi_nbvars') ? (int) $request->get_param('avi_nbvars') : 1,
					'idcateg' 		=> $request->get_param('idcateg') ? (int) $request->get_param('idcateg') : 0,
					'nbimages' 		=> $request->get_param('nbimages') ? (string) $request->get_param('nbimages') : 'all',
					'nbvariations' 	=> $request->get_param('nbvariations') ? (string) $request->get_param('nbvariations') : 'all',
					'spin' 			=> $request->get_param('spin') ? (int) $request->get_param('spin') : 0,
					'attributes' 	=> $request->get_param('attributes') ? (int) $request->get_param('attributes') : 1,
				);
	...
	//====================================================================================
	//== AJAX REQUEST (DEPRECATED - KEPT FOR BACKWARD COMPATIBILITY)
	//====================================================================================
	...
	public function ajax_request() {
		...
		/* 
		I have replaced nonce validation with key validation. When the application is authorized, a unique secret access key is created.
	...
	public function add_product( $product, $pms=array() ) {
		$pms = array_replace_recursive( array(
			...
			// (string) page content of the product
			'page_content' 	=> '',
		...
		if('chrome-extension' === $where_from){
			$product = $this->_product_extract_data( $product )['data'];
		}
		...
		//var_dump('<pre>', $opValidProduct, $product , '</pre>'); echo __FILE__ . ":" . __LINE__;die . PHP_EOL;
		...
		//var_dump('<pre>', $retProd['Variations'] ,$product, '</pre>'); echo __FILE__ . ":" . __LINE__;die . PHP_EOL;
	...
	private function product_price( $product=array(), $pms=array() ) {
		...
		foreach ( $__ as $what ) {
			...
			$__2["{$what}_"] = $this->the_plugin->directimport_get_product_price_format( $$what, $country );
		}
		extract( $__2 );
