...
require_once( WOOZONE_ABSPATH . 'melib/autoload-frontend.php' );
	...
	class WooZoneFrontend
	{
		...
		public function __construct( $parent )
		{
			...
			if ( ! $this->is_admin ) {
				...
				add_action( 'WooZone_header', array($this, 'frontend_custom_header'), 0 );
				add_action( 'WooZone_footer', array($this, 'frontend_custom_footer'), 31 );
			}
		...
		public function init() {
			WooZone_debugbar()->add2bar_menu( 'woozone-debugbar-session-check', WooZone()->_translate_string( 'Session Check' ), array() );
			WooZone_debugbar()->add2bar_menua( 'woozone-debugbar-session-check', WooZone()->_translate_string( 'Session Check' ), array() );
			...
			if ( $is_country_check ) {
				...
				switch ($box_countries_pos) {
					case 'before_add_to_cart':
						...
						if ( 'Kingdom - Woocommerce Amazon Affiliates Theme' == $this->current_theme || 'BravoStore' == $this->current_theme  ) {			
							add_action( 'WooZone_footer', array( $this, 'before_add_to_cart' ), 1 );
						}
						break;
					
					case 'before_title_and_thumb':
						add_action( 'WooZone_footer', array( $this, 'before_title_and_thumb' ), 1 );
						break;

					case 'before_woocommerce_tabs':
						add_action( 'WooZone_footer', array( $this, 'before_woocommerce_tabs' ), 1 );
						break;
				...
				if ( in_array('minicart', $where_country_check) ) {
					...
					if ( 'Kingdom - Woocommerce Amazon Affiliates Theme' == $this->current_theme ) {
						add_action( 'WooZone_footer', array( $this, 'widget_cart_item_quantity' ), 1 );
					}
				}
			...
			if ( $this->product_buy_is_amazon_url && ( 'external' == $this->p_type ) ) {
				/*
				add_action( 'WooZone_footer', array($this->the_plugin, '_product_buy_url_make'), 30 );
		...
		public function wp_enqueue_scripts() {
			...
			if( !wp_script_is($this->alias . '-frontend-script') ) {
				...
				$vars = array(
					...
					'nonce' 			=> wp_create_nonce('WooZone_frontend'),
					'lang' 					=> array(
						...
						'updating'		         				=> WooZone()->_translate_string( 'Updating...' ),
		...
		*/
		...
		public function woocommerce_single_product_summary() {
			...
			if( $show_hox ){
				$box = CountryAvailability::getInstance()->box_country_check_details( $product );
				if ( !empty($box) )
					echo $box;
			}
		}
		...
		public function woocommerce_cart_item_quantity($product_quantity, $cart_item_key, $cart_item=null) {
			...
			$box = CountryAvailability::getInstance()->box_country_check_small( isset($cart_item['product_id']) ? $cart_item['product_id'] : 0 );
			if ( !empty($box) ) {
				//$str .= $box;
		...
		public function widget_cart_item_quantity() {
			...
			$box = CountryAvailability::getInstance()->box_country_check_minicart( $pms );
			if ( !empty($box) )
				echo $box;
		}
		...
		public function woocommerce_product_tabs( $tabs ) {
			$tabs['woozone_tab_countries_availability'] = array(
				'title'				=> WooZone()->_translate_string( 'Countries availability' ),
				'priority'		=> 15,
		...
		public function single_product_summary( $pms=array() ) {
			...
			$box = CountryAvailability::getInstance()->box_country_check_details( $product, $pms );
			if ( !empty($box) )
				echo $box;
		}
		...
		public function box_amazon_shops_checkout( $pms=array() ) {
			...
			//:: get template
			$contents = WooZone_get_template_html( 'amazon/checkout_multishops.php', array_replace_recursive( array(
				'where' 					=> $where,
				'shops' 					=> $shops,
			), $pms ));
		...
		public function redirect_amazon( $redirect_asin='' ) {
			...
			// PER PROVIDER
			if ( 'amazon' == $provider ) {
				//$user_country = CountryAvailability::getInstance()->get_product_country_current( $post_id, array() );
			...
			?>
				<form id="amzRedirect" method="GET" action="<?php echo Utils::getInstance()->getAmazonCartLink($user_country['website']); ?>">
...
<?php
		...
		public function redirect_cart() {
			...
			<form target="<?php echo $checkout_type;?>" id="amzRedirect" method="GET" action="<?php echo Utils::getInstance()->getAmazonCartLink($domain); ?>">
		...
		public function woo_cart_get_amazon_prods_bycountry() {
			...
			foreach ($prods as $key => $value) {
				...
				$product_country = CountryAvailability::getInstance()->get_product_country_current( $prod_id, array() );
	...
	<div class="WooZone-cross-sell-loader">
		...
		<?php
			...
			WooZone_debugbar()->add2bar_menu( 'woozone-debugbar-cross-sell', WooZone()->_translate_string( 'Frequently Bought Together' ), array() );
			WooZone_debugbar()->add2bar_menua( 'woozone-debugbar-cross-sell', WooZone()->_translate_string( 'Frequently Bought Together' ), array() );
		...
		public function _cross_sell_box( $atts=array() ) {
			...
			if ( isset($products['status'], $products['rows']) && 'valid' == $products['status'] && !empty($products['rows']) ) {
				...
				$backHtml[] =   '<h2>' . WooZone()->_translate_string( 'Frequently Bought Together' ) . '</h2>';
				...
				foreach ($products['rows'] as $key => $value) {
					...
					if ( $cc == 0 && ( $asin == $value['ASIN'] || $asin == $value['ParentASIN'] ) ) {
						...
						$backHtml[] =           '<div class="cross-sell-product-title"><strong>' . WooZone()->_translate_string( 'This item' ) . ': </strong>' . $value['Title'] . '</div>';
		...
		public function amazon_reviews_custom_product_tabs( $tabs=array() ) {
			...
			if ($this->amazon_reviews_product_has_custom_tabs($product)) {
				...
				foreach ($this->woo_tab_data as $tab) {
					...
					$tabs[ $tab['id'] ] = array(
						'title'    => WooZone()->_translate_string( 'Amazon Customer Reviews' ),
						'priority' => $priority,
			...
			if ( $this->woo_tab_data === false ) {
				...
				if ( isset($reviews, $reviews[0], $reviews[0]['content']) ) {
					...
					if ( ! empty($reviews[0]['content']) ) {
						...
						$user_country = CountryAvailability::getInstance()->get_product_country_current( $prod_id, array() );
						if ( empty($user_country) || ! isset($user_country['website']) ) {
							$user_country = GeoLocation::getInstance()->get_country_perip_external();
							$user_country = $user_country['user_country'];
						}
		...
		public function get_asin_first_variation( $product_id ) {
			...
			return WooMisc::getInstance()->get_asin_first_variation( $product_id );
		}
		...
		public function action_do_product_page() {
			...
			if ( 'yes' == $this->syncfront_activate ) {
				...
				add_action( 'WooZone_footer', array( $this, 'syncfront_wrapper' ), 1 );
			}
		...
		public function amz_disclaimer_price_html( $price, $product ) {
			...
			if ( ! is_array($text) ) {
				...
				$ret = array();
				$ret[] = '<em class="WooZone-price-info">';
		...
		private function _amz_disclaimer_price_html( $post_id, $pms=array() ) {
			...
			$details_link = sprintf(
				'<a name="' . $details_text . '" href="#TB_inline?&inlineId=WooZone-price-disclaimer&width=500&height=250" class="thickbox link">' . $details_text . '</a>' );

			$details_content = array();
			$details_content[] = '<div id="WooZone-price-disclaimer" style="display: none;">';
			$details_content[] = 	'<p></p>';
			$details_content[] = 	'<p>';
			...
			if ( 'amazon' !== $provider ) {
				$text[] = '&nbsp;';
				$text[] = '<em class="WooZone-price-info">';
				$text[] = sprintf(
			...
			if ( 'yes' == $this->the_plugin->frontend_show_coupon_text ) {
				...
				if ( ! empty($promotion) && is_array($promotion) && isset($promotion['PromotionId']) ) {
					...
					$coupon = array(
						...
						'details'           => sprintf( __($this->the_plugin->_translate_string( 'Your coupon will be applied at amazon checkout.' ) . ' %s', $this->localizationName), '<a name="' . __($this->the_plugin->_translate_string( 'COUPON DETAILS' ), $this->localizationName) . '" href="#TB_inline?width=500&height=700&inlineId=WooZone-coupon-popup" class="thickbox link">' . __($this->the_plugin->_translate_string( 'Details' ), $this->localizationName) . '</a>' ),
					...
					<div id="WooZone-coupon-popup" style="display: none;">
						<div class="WooZone-coupon-container">
							<div class="WooZone-coupon-header">
								<p><?php echo WooZone()->_translate_string( 'Coupons available for this offer' ); ?></p>
								<a href="<?php echo $coupon['link_more']; ?>" target="_blank"><?php echo WooZone()->_translate_string( 'View more coupons' ); ?></a>
							</div>
							...
							<div class="WooZone-coupon-summary">
								<div class="WooZone-coupon-summary-inner">
									<div class="WooZone-coupon-summary-inner-left">
										<a href="<?php echo $coupon['link']; ?>" target="_blank"><?php echo WooZone()->_translate_string( 'Your coupon' ); ?></a>
									</div>
		...
		public function cached_terms_additional_information_tab( $tabs ) {
			$tabs['additional_information']['title'] = WooZone()->_translate_string( 'Additional Information' );
		...
		public function syncfront_wrapper() {
			...
			$html[] = 		'<h3>';
			$html[] =		sprintf( __( $this->the_plugin->_translate_string( 'We`ve just updated this product information. The page will auto refresh in about <span>%s</span> seconds.' ), 'WooZone'), $refresh_in );
			$html[] = 		'</h3>';
			...
			WooZone_debugbar()->add2bar_menu( 'woozone-debugbar-sync-frontend', WooZone()->_translate_string( 'Product Synchronization' ), array() );
			WooZone_debugbar()->add2bar_menua( 'woozone-debugbar-sync-frontend', WooZone()->_translate_string( 'Product Synchronization' ), array() );
		...
		public function ajax_requests()
		{
			...
			$nonce = isset($_REQUEST['nonce']) ? $_REQUEST['nonce'] : 'invalid';
			if ( !wp_verify_nonce( $nonce, 'wp_ajax_WooZone_frontend' )) {
			...
			$allowed_action = array(
				//'save_countries',
				//'update_countries',
				//'save_product_country',
				//'load_cross_sell',
				//'cross_sell_empty_cache',
				//'load_amazon_reviews',
				'do_sync'
			);
			...
			if ( 'do_sync' == $action ) {
				...
				if ( empty($asin) ) {
					$asin = WooZone_get_post_meta($id, '_amzASIN', true);
				}
