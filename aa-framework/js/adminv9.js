...
WooZone = (function ($) {
	...
	function makeRequest( callback )
	{
		...
		if( ajaxBox.length == 0 ) return false;
	...
	function hashChange_old2() {
		if (location.hash != "") {
			...
			if( t.length > 0 && location.search == "?page=WooZone" ){
				makeRequest();
			}
		}
	}
    ...
    function makeTabs()
    {
    		var initialTabIdx = 0; //7; //DEBUG - geo location
        ...
        $('ul.tabsHeader').each(function() {
            ...
            $active = $( $links.filter('[title="'+__currentTab+'"]')[initialTabIdx] || $links[initialTabIdx] );
	...
	function triggers()
	{
	    $('.WooZone-responsive-menu').click(function() {
	        $('.WooZone-nav').toggle('fast');
	    });
		...
		$("form#events-noaws_sync_widget").each( function(){
			...
			$("body").on( 'click', 'form#events-noaws_sync_widget #filter-by-button', function(e){
				...
				form.find('.WooZone-filter-by').remove();

				form.submit();
			});

			$('body').on('click', '.WooZone-view-all-asin a', function(e){
				e.preventDefault();
			...
			$('body').on('click', '.WooZone-view-all-statusmsg a', function(e){
				e.preventDefault();
			...
			$('body').on('click', '.WooZone-sync-now', function(e){
				e.preventDefault();
	...
	//== EBAY
	//====================================================================
    function ebayAuthorizeAPI()
    {
        $('body').on('click', '.WooZoneAuthorizeEbayAPI', function (e) {
            e.preventDefault();

            $('#ebay_DEVID').val( $.trim( $('#ebay_DEVID').val() ) );
            $('#ebay_AppID').val( $.trim( $('#ebay_AppID').val() ) );
            $('#ebay_CertID').val( $.trim( $('#ebay_CertID').val() ) );
            $('#ebay_runame').val( $.trim( $('#ebay_runame').val() ) );
            $('.WooZone-aff-ids input').each(function() {
                $(this).val( $.trim( $(this).val() ) );
            });
            
            var client_id = $.trim( $('#ebay_AppID').val() );
            var runame = $.trim( $('#ebay_runame').val() );
            ...
            that.removeClass('blue').addClass('gray');
            that.val('Authorizing API ...');
            
            saveOptions(submit_btn, function(){
                
                var auth_params = 'client_id=' + client_id;
                auth_params += '&redirect_uri=' + runame;
                auth_params += '&response_type=code';
                auth_params += '&scope=' + encodeURIComponent('https://api.ebay.com/oauth/api_scope');
                var auth_redirect = 'https://auth.ebay.com/oauth2/authorize?' + auth_params;
    ...
    {
		$('body').on('click', '.WooZoneCheckKeysEbay', function (e) {
			e.preventDefault();
            ...
            saveOptions(submit_btn, function(){
                ...
                jQuery.post(ajaxurl, {
                    'action' : 'WooZoneCheckKeysEbay'
                        ...
                        if( response.status == 'valid' ){
                            alert('WZone was able to connect to Ebay with the specified Keys pair.');
                        }
                        else{
                            var msg = 'WZone was NOT able to connect to Ebay with the specified Keys pair.' + "\n" + 'Please triple-check your Keys.';
