...
require_once( WOOZONE_ABSPATH . 'aa-framework/wzone-ai-descriptions.php');
...
if(class_exists('WooZone') != true) {
	class WooZone {
		...
		public function addNewProduct( $retProd=array(), $pms=array() )
		{
			...
			add_action('wzone_product_imported', 'wzone_add_ai_description', 10, 2);

	/* cod AI */
	function wzone_add_ai_description($post_id, $product_data) {
	    $title = get_the_title($post_id);
	    $bullets = isset($product_data['features']) ? $product_data['features'] : [];

	    $ai_desc = wzone_generate_ai_description($title, $bullets);
	    if ($ai_desc) {
	        wp_update_post([
	            'ID' => $post_id,
	            'post_content' => wp_kses_post($ai_desc)
	        ]);
	    }

	    $options = get_option('save_options'); // sau 'wzone_config'
		if (empty($options['wzone_use_ai_descriptions'])) {
		    return; // AI dezactivat → ieși din funcție
		}
	}
		...
		public function updateWooProduct( $retProd=array(), $pms=array() )
		{
			...
			else if ( 'amazon' == $provider && in_array($this->amzapi, array('newapi', 'creatorsapi')) ) {
				foreach ( $_sync_rules as $kk => $vv ) {
					if ( ! in_array($kk, array(
		...
		public function framework_style( $onlygenerate=false )
		{
			...
			try {
				...
				//die( var_dump( "<pre>", $buffer  , "<pre>" ) . PHP_EOL .  __FILE__ . ":" . __LINE__  ); 
 ...
 <strong>GProducts allows you to embed and display several Amazon products and provide relevant information – product details – rating, price, description, free shipping, prime ready & more into a great looking box! </strong>  <a href="https://1.envato.market/kEbD3" target="_blank"> GProducts - Amazon Affiliates Products Boxes Block </a> <strong style="color:#009f1e;">Get it for only 15$! </strong>
			...
			if( isset($_REQUEST['ebay_auth']) ) {
				if( $_REQUEST['ebay_auth'] == 'accepted') {
					...
					$_errors[] = '<h3 style="color:red;">Invalid eBay auth response!</h3>'. '<pre>' . ( isset($_REQUEST['ebay_auth_response']) ? esc_attr( base64_decode($_REQUEST['ebay_auth_response']) ) : '' ) .'</pre>';
				}
			}
				...
				foreach ($this->cfg['freamwork-js-files'] as $key => $value) {
					...
					if ( is_file($this->cfg['paths']['freamwork_dir_path'] . $value) ) {
						if( in_array( $key, $javascript ) ) {
							...
							if( $key == 'admin' ){
								wp_localize_script( $this->alias . '-' . $key, 'WooZoneHelper', [
									'nonce' => wp_create_nonce('woozone_admin')
								]);
							}
						}
					} else {
		...
		*/
		...
		public function get_ws_object_new( $provider, $what='helper', $pms=array() ) {
			...
			else if ( 'amazon' == $provider && 'creatorsapi' === $this->amzapi ) {
				$provider_4ws .= 'Creators';
			}
		...
		public function product_by_asin( $asins=array() ) {
			...
			$sql_asin2id = $wpdb->prepare("select pm.meta_value as asin, p.* from " . $wpdb->prefix.'posts' . " as p left join " . $wpdb->prefix.'postmeta' . " as pm on p.ID = pm.post_id where 1=1 and !isnull(p.ID) and pm.meta_key = '$key' and pm.meta_value != '' and pm.meta_value in (%s);", $asins_);

			//die( var_dump( "<pre>", $sql_asin2id , "<pre>" ) . PHP_EOL .  __FILE__ . ":" . __LINE__  ); 
			...
			// because we have old amazon products which have only '_amzASIN' meta (they don't have this new '_amzaff_prodid' meta)
			$sql_asin2id = $wpdb->prepare("select pm.meta_value as asin, p.* from " . $wpdb->prefix.'posts' . " as p left join " . $wpdb->prefix.'postmeta' . " as pm on p.ID = pm.post_id where 1=1 and !isnull(p.ID) and pm.meta_key = '$_key' and pm.meta_value != '' and pm.meta_value in (%s);", $asins_);
			$res_asin2id = $wpdb->get_results( $sql_asin2id, OBJECT_K );
			//var_dump('<pre>', $res_asin2id , '</pre>'); echo __FILE__ . ":" . __LINE__;die . PHP_EOL;
			if ( !empty($res_asin2id) ) {
				foreach ($res_asin2id as $k => $v) {
		...
		public function init_plugin_attributes() {
			...
			$this->amzapi = isset($this->amz_settings['amzapi'])
				&& in_array($this->amz_settings['amzapi'], array('oldapi', 'newapi', 'creatorsapi'))
		...
		public function provider_action_controller( $action, $provider, $pms=array() ) {
			...
			//var_dump('<pre>', $action, $provider, $pms, '</pre>'); echo __FILE__ . ":". __LINE__;die . PHP_EOL;
			$pms = array_replace_recursive( array(
		...
		private function _directimport_get_price_format_bycountry( $amount, $country ) {
			...
			if ( in_array( $country, array('de', 'it', 'es', 'fr', 'com.br', 'nl', 'com.tr', 'se', 'pl', 'ca') ) ) {
