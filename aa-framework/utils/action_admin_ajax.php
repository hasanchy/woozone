	...
	class WooZone_ActionAdminAjax
	{
		...
		*/
		public function __construct( $parent )
		{
			...
			// ajax requests
			add_action('wp_ajax_WooZone_CleanLogTables', array( $this, 'db_clean_log_tables' ));
			...
			//add_action('wp_ajax_WooZone_woocustom', array( $this, 'woocustom_ajax_actions') );

			add_action('wp_ajax_WooZone_GeoLocation', array( $this, 'ajax_geolocation' ));
		}
		...
		*/
		...
		/*public function woocustom_ajax_actions( $retType='die' ) {
			...
			$obj = new WooZoneWooCustom();

			$obj->ajax_requests();
		}*/

		public function ajax_geolocation( $retType='die' ) {
			if( !current_user_can('administrator') ){
				die( __( "Only administrator can update this option!", "WooZone" ));
			}
			...
			if ( ! in_array($action, array('check-all')) ) {
				die(json_encode($ret));
			}

			if ( 'check-all' === $action ) {
				//usleep(10000000); //DEBUG/ sleep in microseconds
				$opStatus = GeoLocation::getInstance()->check_all(array('dosave' => true));
				$html = GeoLocation::getInstance()->get_box_list_services( array());
				$ret = array_replace_recursive($ret, array(
					'status' => 'valid',
					'html' => $html,
					'ip' => $opStatus['ip'],
