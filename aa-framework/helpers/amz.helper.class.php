...
if ( class_exists('WooZoneAmazonHelper') != true ) { class WooZoneAmazonHelper extends WooZone {
	...
	public function is_amazon_valid_response( $response, $operation='' ) {
		...
		$ret = array(
			...
			// woozone custom codes:
			// 		woozone:aws.init.issue | woozone:aws.request.dropped |
			// 		woozone:aws5.issue
			'amz_code' 		=> '',
			'last_request'  => null,
			'last_response' => null,
		...
		if (isset($response['last_request'])) $ret['last_request'] = $response['last_request'];
		if (isset($response['last_response'])) $ret['last_response'] = $response['last_response'];
		if (isset($response['nb_amz_req'])) $ret['nb_amz_req'] = $response['nb_amz_req'];
		...
		if ( isset($response['status']) && ( 'invalid' == $response['status'] ) ) {
			...
			$merged = array_merge($ret, array(
				'msg'       => $msg,
				'html'      => $msg,
				'code'      => isset($response['code']) ? $response['code'] : 1,
				'amz_code' 	=> $amz_code,
			));
			if (isset($response['last_request'])) $merged['last_request'] = $response['last_request'];
			if (isset($response['last_response'])) $merged['last_response'] = $response['last_response'];
			if (isset($response['nb_amz_req'])) $merged['nb_amz_req'] = $response['nb_amz_req'];
			return $merged;
		}
	...
	public function stress_test()
	{
		...
		WooZone_session_start();

		if( $action == 'import_images' ){

			//var_dump('<pre>', $_SESSION, '</pre>'); //echo __FILE__ . ":" . __LINE__;die . PHP_EOL;

			$product_id = isset($_SESSION["WooZone_test_product_local_id"])
				? $_SESSION["WooZone_test_product_local_id"] : 0;

			if( isset($_SESSION["WooZone_test_product"]) && count($_SESSION["WooZone_test_product"]) > 0 && $product_id ){
		...
		if( $action == 'insert_product' ){
			if( isset($_SESSION["WooZone_test_product"]) && count($_SESSION["WooZone_test_product"]) > 0 ){
				...
				if ( $insert_id ) {
					...
					$_SESSION["WooZone_test_product_local_id"] = $insert_id;
		...
		if( is_array( $sql_search_index_by_country ) && ! empty($sql_search_index_by_country) ) {
			foreach( $sql_search_index_by_country as $search_index ) {
				...
				if ( in_array($this->the_plugin->amzapi, array('newapi', 'creatorsapi')) ) {
					//[new in api v5 / creatorsapi]
					//"Actor#Artist#Author#Availability#Brand#BrowseNode#Condition#CurrencyOfPreference#DeliveryFlags#LanguagesOfPreference#MaxPrice#Merchant#MinPrice#MinReviewsRating#MinSavingPercent#Sort#Title"
					$search_index->itemSearchParams .= "#Brand#Condition#DeliveryFlags#MinReviewsRating#MinSavingPercent#Title";
				}

				$categs[$key] = explode( '#', $search_index->itemSearchParams );
				$categs[$key] = array_unique( array_filter( $categs[$key] ) );
			}
		}
	...
	public function getBrowseNodesList( $nodeid=0 ) {
		...
		$prefix_opt = $provider . '_' . $country . '_';

		$optname = $this->the_plugin->alias . '_node_children_' . $prefix_opt . $nodeid;
		...
		//var_dump('<pre>', $nodeid, $nodes , '</pre>'); echo __FILE__ . ":" . __LINE__;die . PHP_EOL;
		return $nodes;
	}
	...
	public function getProductDataFromAmazon( $retType='die', $pms=array() ) {
		...
		// [DISABLED] - issues with variations after search. The data in cache does not contain variations information - 03.10.2022
		// from cache
			...
			try {
				...
				if ( $respStatus['status'] != 'valid' ) { // error occured!
					...
					if ( 1 ) {
						...
						// build product data array
						$thisProd = isset($thisProd[0]) ? $thisProd[0] : $thisProd;
						$retProd = array(); 
						$retProd = $this->build_product_data( $thisProd );
				...
				$excmsg = WooZoneGetExceptionMsg( $e );
	...
	public function set_woocommerce_attributes( $itemAttributes=array(), $post_id=null ) 
	{
		global $wpdb;
		global $woocommerce;
	...
	public function set_woocommerce_variations( $retProd, $parent_id, $pms=array() )
	{
		...
		$_vardim = isset($retProd['Variations']['VariationDimensions']['VariationDimension'])
			? $retProd['Variations']['VariationDimensions']['VariationDimension'] : null;
		if ( ! is_null($_vardim) ) {
			if ( is_array($_vardim) && ! empty($_vardim) ) {
				foreach ($_vardim as $dim) {
					$VariationDimensions[$dim] = array();
				}
			}
			else if ( is_string($_vardim) ) {
				$VariationDimensions[$_vardim] = array();
			}
		}
		...
		foreach ($variations as $variation_item) {
			...
			$stat_variation_post = $this->variation_post( $variation_item, $parent_id, $VariationDimensions, $pms );
	...
	public function variation_post( $variation_item, $parent_id, $VariationDimensions, $pms )
	{
		...
		//:: 02.09.2022 - add selected attributes to product variation
		//---------------------
		//:: import attributes
		/*if ( $pms['import_attributes'] ) {
			if ( count($variation_item['ItemAttributes']) > 0 ) {
				$this->the_plugin->timer_start(); // Start Timer
			}

			// add product attributes
			$this->set_woocommerce_attributes( $variation_item['ItemAttributes'], $variation_post_id );
		...
		if(is_array($variation_item['VariationAttributes']['VariationAttribute']) && isset($variation_item['VariationAttributes']['VariationAttribute'][0]['Name'])) {
			...
			foreach ($variation_item['VariationAttributes']['VariationAttribute'] as $va) {
				...
				$dmName = isset($va['Name']) ? $va['Name'] : '';
				$dmValue = isset($va['Value']) ? $va['Value'] : '';
	...
	public function get_countries( $key='country' ) {
		...
		if ( 'country' == $key ) {
			return  array(
				...
				'ae' => __('UAE', $localizationName),
				'nl' => __('Netherlands', $localizationName),
				'sg' => __('Singapore', $localizationName),
				'sa' => __('Saudi Arabia', $localizationName),
				'com.tr' => __('Turkey', $localizationName),
				'se' => __('Sweden', $localizationName),
				'pl' => __('Poland', $localizationName),
				'eg' => __('Egypt', $localizationName),
			);
		}
			...
			return  array(
				...
				'ae' => '<a href="https://affiliate-program.amazon.ae" target="_blank">UAE</a>',
				'nl' => '<a href="https://partnernet.amazon.nl" target="_blank">Netherlands</a>',
				'sg' => '<a href="https://affiliate-program.amazon.sg" target="_blank">Singapore</a>',
				'sa' => '<a href="https://affiliate-program.amazon.sa" target="_blank">Saudi Arabia</a>',
				'tr' => '<a href="https://gelirortakligi.amazon.com.tr" target="_blank">Turkey</a>',
				'se' => '<a href="https://affiliate-program.amazon.se" target="_blank">Sweden</a>',
				'pl' => '<a href="https://affiliate-program.amazon.pl" target="_blank">Poland</a>',
				'eg' => '<a href="https://affiliate-program.amazon.eg" target="_blank">Egypt</a>',
			);
		}
		return array();
	}
	...
	public function api_search_bypages( $pms=array() ) {
		...
		//var_dump('<pre>',$__asinsDebug,'</pre>');
			...
			$response = array('status' => 'invalid', 'msg' => $msg, 'code' => 1, 'amz_code' => strtolower(WooZoneGetExceptionCode( $e )), 'req_link' => $req_link);
		...
		/*
		* function: apiv5_addVariationsToResponse
		* Disabled on 05.10.2021 because of many requests and big load time when searching.
		*/
	...
	public function api_make_request( $pms=array() ) {
		...
		if (isset($request_status['last_request'])) $ret['last_request'] = $request_status['last_request'];
		if (isset($request_status['last_response'])) $ret['last_response'] = $request_status['last_response'];
		if (isset($request_status['nb_amz_req'])) $ret['nb_amz_req'] = $request_status['nb_amz_req'];
		return $ret;
	}
