...
if(class_exists('WooZoneGenericHelper') != true) {
	class WooZoneGenericHelper extends WooZone
	{
 ...
 WHERE 1=1 AND a.term_id = %s;";
		...
		public function fix_node_childrens( $retType='die' ) {
			...
			if ( 'fix_node_childrens' == $action ) {
				//WooZone_node_children_amazon_co.uk_1
				$sql = "DELETE FROM $wpdb->options WHERE option_name LIKE 'WooZone_node_children_%';";  
				$query = $wpdb->query($sql);

				//WooZone_ebay_EBAY-IT_node_children_-1
				$sql = "DELETE FROM $wpdb->options WHERE option_name LIKE 'WooZone_ebay_EBAY-%';";  
				$query = $wpdb->query($sql);
		...
		public function fix_issues( $retType='die' ) {
			...
			if ( 'reset_sync_stats' == $action ) {
				...
				$optionsList = array(
					...
					'WooZone_sync_last_bulk_code',
					'WooZone_sync_fix_samebulk',
					...
					'WooZone_sync_witherror_last_bulk_code',
					'WooZone_sync_witherror_fix_samebulk',
			...
			if ( 'refresh_categories' == $action ) {

				$opStat = $this->_refresh_categories();
				$ret['msg_html'][] = $opStat['msg_html'];
			}
		...
		public function _refresh_categories() {
			global $wpdb, $WooZone;
			...
			if ( ! in_array($what, array('main_categories', 'browse_nodes', 'mc_bn', 'ebay_categories')) ) {
				return array_replace_recursive( $ret, array(
					'msg_html' => 'wrong chosen action!',
				));
			}

			//:: EBAY
			if ( in_array($what, array('ebay_categories')) ) {

				//WooZone_ebay_EBAY-IT_node_children_-1
				$sql = "DELETE FROM $wpdb->options WHERE option_name LIKE 'WooZone_ebay_EBAY-%';";  
				$res = $wpdb->query($sql);

				$msg[] = 'ebay categories: ' . (false === $res ? 'query error!' : sprintf( '%s rows cleaned!', (int) $res ));
			}

			//:: AMAZON
			if ( in_array($what, array('browse_nodes', 'mc_bn')) ) {

				//WooZone_node_children_amazon_co.uk_1
				$sql = "DELETE FROM $wpdb->options WHERE option_name LIKE 'WooZone_node_children_%';";  
				$res = $wpdb->query($sql);

				$msg[] = 'amazon browse nodes: ' . (false === $res ? 'query error!' : sprintf( '%s rows cleaned!', (int) $res ));
			}

			if ( in_array($what, array('main_categories', 'mc_bn')) ) {

				require_once( $this->the_plugin->cfg['paths']['plugin_dir_path'] . '/modules/setup_backup/default-sql.php' );

				// empty table with categories
				$table = $wpdb->prefix . 'amz_locale_reference';
				$sql = trim("TRUNCATE TABLE $table;");
				$res = $wpdb->query($sql);

				$msg[] = 'amazon main categories: ';
				$msg[] = (false === $res ? 'query error!' : 'table cleaned!');

				// re-populate table with categories
				@ini_set('memory_limit', '512M');
				@set_time_limit ( 0 );

				$filename_tables_data = $this->the_plugin->cfg['paths']['plugin_dir_path'] . 'modules/setup_backup/db/tables_data.sql';
				$opStat = WooZoneSetupBackup::getInstance()->install_tables_data($filename_tables_data);

				$msg[] = (false === $opStat ? 'tables data file not found!' : 'categories added!');
			}

			return array_replace_recursive( $ret, array(
				'msg_html' => implode(' | ', $msg),
			));
		}
...
select p.ID from {$wpdb->posts} as p
	...
	where 1=1
		...
		and ( 
			( ( pm2.meta_key = '_regular_price' and pm2.meta_value = '' ) and ( pm3.meta_key = '_price' and pm3.meta_value = '' ) ) OR 
			( ( pm2.meta_key = '_regular_price' and pm2.meta_value = '0.00' ) and ( pm3.meta_key = '_price' and pm3.meta_value = '0.00' ) ) OR 
			( ( pm2.meta_key = '_regular_price' and pm2.meta_value = '0' ) and ( pm3.meta_key = '_price' and pm3.meta_value = '0' ) )
		)
	order by p.ID asc
;
			";
 ...
 WHERE 1=1 AND a.term_taxonomy_id IN (%s) AND a.taxonomy = '%s' ORDER BY a.term_taxonomy_id ASC;";
		...
		public function log_tables_clean( $tables, $clean_option ) {
			...
			foreach ( $tables as $table ) {
				$table = trim($table);
				if ( strpos($table, 'amz_') !== 0 && strpos($table, 'woozone_') !== 0 ) {
					continue;
				}
				$table_size = $WooZone->WooZone_show_table_status( $table );
