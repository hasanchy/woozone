...
if ( class_exists('WooZoneEbayHelper') != true ) { class WooZoneEbayHelper extends WooZone {
	...
	public function __construct( $the_plugin=array(), $params=array() )
	{
		...
		// ajax actions
		add_action('wp_ajax_WooZoneAuthorizeEbayAPI', array( $this, 'auth_ebay_api'), 10, 2);

		add_action('wp_ajax_WooZoneCheckKeysEbay', array( $this, 'check_keys'), 10, 2);
		add_action('wp_ajax_WooZoneImportProductEbay', array( $this, 'getProductDataFromAmazon' ), 10, 2);

		add_action('wp_ajax_WooZoneStressTestEbay', array( $this, 'stress_test' ));
		...
		if ( in_array($isc, array('details', 'details-only')) ) {
			...
			foreach ($metas as $key) {
				if ( isset($item["$key"]) && !empty($item["$key"]) ) {
					...
					$__value = isset($item["$key"]) ? $item["$key"] : '';
					if ( in_array($key, array('ConvertedCurrentPrice', 'CurrentPrice', 'MinimumToBid')) ) {
						if (
							is_array($__value)
							&& isset($__value[0])
							//&& isset($__value['@attributes'])
						) {
							$__value = $__value[0];
						}
					}
					$retProd['__extra']["$key"] = $__value;
				}
			}
		}
	...
	public function auth_ebay_api( $retType='die', $pms=array() )
	{
		if( !current_user_can('administrator') ){
			die( __( "Only administrator can update this option!", "WooZone" ));
		}
		...
		if( isset($_REQUEST['action']) ) {
			if( $_REQUEST['result'] == 'success' ) {
				...
				if( isset($_REQUEST['code']) && trim($_REQUEST['code']) != '' ) {
					$ebay_req = $this->the_plugin->ebay_utils->get_ebay_auth( $_REQUEST['code'], false, $this->the_plugin->amz_settings );

					if( isset($ebay_req) && trim($ebay_req) != '' ) {
						$_data = json_decode($ebay_req, true);
						if( isset($_data['access_token']) ) {
							if( update_option('WooZone_ebay_auth', $_data) ) {
								wp_redirect( admin_url('admin.php?page=WooZone&ebay_auth=accepted#!/amazon') );
							}
						}else{
							wp_redirect( admin_url('admin.php?page=WooZone&ebay_auth=invalid&ebay_auth_response='.base64_encode(json_encode($_data)).'#!/amazon') );
						}
					}
				}
			}elseif ( $_REQUEST['result'] == 'declined' ) {
				wp_redirect( admin_url('admin.php?page=WooZone&ebay_auth=declined#!/amazon') );
			}
		}
		die();
	}
	...
	public function set_product_meta_options( $retProd, $post_id, $is_variation=true )
	{
		...
		//:: 05.11.2021
		//:: Save seller username for account deletion/closure notifications
		if( isset($retProd['__extra']['Seller']) && isset($retProd['__extra']['Seller']['UserID']) ) {
			update_post_meta($post_id, '_ebay_UserID', $retProd['__extra']['Seller']['UserID']);
		}
	...
	public function api_search_bypages( $pms=array() ) {
		...
		//var_dump('<pre>',$response,'</pre>'); die;
			...
			$response = array('status' => 'invalid', 'msg' => $msg, 'code' => 1, 'amz_code' => strtolower(WooZoneGetExceptionCode( $e )));
