...
<?php
		...
		public function show_search_params( $search_params ) {
			...
			$provider = isset($search_params['extra_params'], $search_params['extra_params']['provider'])
				? $search_params['extra_params']['provider'] : '';
			...
			foreach ( $search_params_ as $k => $v ) {
				...
				foreach ($v as $key => $val) {
					...
					if ( 'country' == $key ) {
						$val = isset($this->providers_countries->countries["$provider"]["$val"]) ? $this->providers_countries->countries["$provider"]["$val"] : $val;
					}
					if ( 'recurrency' == $key ) {
		...
		public function print_schedule_button( $pms=array(), $print=true ) {
			...
			if ( isset($ver2) && $ver2 ) {
				$asset = '<div class="WooZone-add-to-schedule WooZone-form-button WooZone-form-button-success" data-provider="' . $provider . '">' . $title . '</div>';
			}
			else {
				$asset = '<li class="button-block">
					<input type="button" value="' . $title . '" class="WooZone-form-button WooZone-form-button-success WooZone-add-to-schedule" data-provider="' . $provider . '" />
				</li>';
			}
...
<?php
		...
		public function schedule_search_get_params( $pms=array() ) {
			...
			$def_main_aff_id = 'com'; $def_country = 'com';
			if ( 'ebay' == $provider ) {
				$def_main_aff_id = 'EBAY-US'; $def_country = $def_main_aff_id;
			}
			...
			// country
			$country = isset($this->settings[$prefix.'country']) ? (string)$this->settings[$prefix.'country'] : $def_country;
			if ( empty($requestData['country']) ) {
				$requestData['country'] = $country;
			}
			$requestData['_country'] = isset($this->providers_countries->countries["$provider"]["$country"])
				? $this->providers_countries->countries["$provider"]["$country"] : $country;
			
			// main_aff_id
			$main_aff_id = isset($this->settings[$prefix.'main_aff_id']) ? (string)$this->settings[$prefix.'main_aff_id'] : $def_main_aff_id;
			if ( empty($requestData['main_aff_id']) ) {
				$requestData['main_aff_id'] = $main_aff_id;
			}
			$requestData['_main_aff_id'] = isset($this->providers_countries->main_aff_ids["$provider"]["$main_aff_id"])
				? $this->providers_countries->main_aff_ids["$provider"]["$main_aff_id"] : $main_aff_id;
		...
		public function auto_save_queue( $pms=array() ) {
			...
			$requestData = array(
				...
				'country_group'		 => isset($_REQUEST['country']) ? $_REQUEST['country'] : '',
				'provider_group'	 => isset($_REQUEST['provider']) ? $_REQUEST['provider'] : '',
			...
			if ( !empty($asins) ) {
				...
				// because in amz_queue table we'll have asins which have no prefix for amazon: B075QY2S4Z | amz-B075QY2S4Z
				// so both cases must be checked if asin exists
				$tmp_asins = $this->the_plugin->prodid_set($asins, 'amazon', 'sub');
				$asins_ += $tmp_asins;

				$asins_ = array_unique(array_filter($asins_));

				$asins_ = implode(',', array_map(array($this->the_plugin, 'prepareForInList'), $asins_));

				$sql = "select a.id, a.asin, a.provider from " . $table_name . " as a where 1=1 and a.asin in ($asins_) and a.from_op = %s;";
				...
				if ( !empty($res) ) {
					foreach ($res as $k => $v) {
						...
						$_asin = $v->asin;
						if ( 'amazon' == $v->provider || empty($v->provider) ) {
							$_asin = $this->the_plugin->prodid_set($_asin, 'amazon', 'add');
						}
			...
			foreach ($asins as $asin) {
				...
				if ( !isset($asins_db["$asin"]) ) {
					...
					if ( isset($asins_post["$asin"]) && ! empty($asins_post["$asin"]) && isset($asins_post["$asin"]->ID) ) {
					...
					// does NOT exists in wp_posts
						...
						//var_dump('<pre>',$asin, $provider, $country ,'</pre>'); continue 1;
						...
						if ( $initHereInsane ) {
							...
							$pmsSetupHelper = array(
								...
								$prefix.'main_aff_id'			=> $this->the_plugin->main_aff_id(),
							);
							$WooZoneInsaneImport->setupAmazonHelper( $pmsSetupHelper, array(
								'provider' 	=> $provider,
							));
						}

						// status: new, failed, done, running
						$product_info = $this->get_product_info( $asin, array(
		...
		public function cronjob_queue( $pms, $return='die' ) {
			...
			foreach ( $providers as $provider ) {
				...
				if ( 'invalid' == $provider_status['status'] ) {
					...
					$amz_verify_sql .= " and a.provider != '$provider' ";
				}
			}
			...
			$sql_new = "SELECT count(a.id) as nb FROM " . $table . " as a WHERE 1=1 $amz_verify_sql AND a.status = 'new' AND a.nb_tries < %s;";
			$sql_new = $wpdb->prepare( $sql_new, self::$max_nb_tries );
			$res_new = $wpdb->get_var( $sql_new );
			$cron_stats_db['queue_rows_new'] = (int) $res_new;

			//self::$queue_chunk_nb = 30; //DEBUG
			//self::$max_nb_tries = 1; //DEBUG

			$sql = "SELECT a.* FROM " . $table . " as a WHERE 1=1 $amz_verify_sql AND a.status IN ('new', 'error') AND a.nb_tries < %s ORDER BY a.nb_tries ASC, a.id ASC LIMIT 0, ".self::$queue_chunk_nb.";";
			...
			if (empty($res)) {
				...
				$ret = array_merge($ret, array(
					'status' => 'done',
					'msg' 	=> 'no products found to be (auto) imported - db step.' . $amz_verify_msg,
				));
				return $ret;
			}
			...
			foreach ($prods as $key => $row) {
				...
				$import_params['from_op'] = $row['from_op'];
				$import_params['country'] = $row['country'];
			...
			if (empty($prods)) {
				$ret = array_merge($ret, array(
					'status' => 'done',
					'msg' 	=> 'no products found to be (auto) imported - queue step.' . $amz_verify_msg,
				));
				return $ret;
			}
			...
			if ( ! empty($amz_verify_msg) ) {
				$mainstatus_msg[] = $amz_verify_msg;
			}

			$WooZoneInsaneImport->when_importing_starts( 'return' ); // recount terms

			//:: main foreach
			foreach ($prods as $key => $row) {
				$status_msg = array();

				$prefix = 'amazon' != $row['provider'] ? $row['provider'].'_' : '';

				//:: init import object
				$this->the_plugin->cur_provider = $row['provider'];

				$pmsSetupHelper = array(
					$prefix.'country' 				=> $row['country'],
					$prefix.'main_aff_id'			=> $this->the_plugin->main_aff_id(),
				);
				$WooZoneInsaneImport->setupAmazonHelper( $pmsSetupHelper, array(
					'provider' 	=> isset($row['provider']) ? $row['provider'] : 'amazon',
				));
				//var_dump('<pre>', $WooZoneInsaneImport->ebayHelper->aaWooZoneEbayWS->getcfg() , '</pre>'); continue;
				//var_dump('<pre>', $WooZoneInsaneImport->amazonHelper->aaAmazonWS->getcfg() , '</pre>'); continue;
				...
				if ( ( $product_id > 0 )
					...
					&& isset($import_stat['product_data'], $import_stat['product_data']['Title'])
				) {
					$product_title = $import_stat['product_data']['Title'];
				}
			...
			$WooZoneInsaneImport->after_importing_last_product( 'return' ); // recount terms
		...
		public function cronjob_search( $pms, $return='die' ) {
			...
			if (empty($res)) {
				...
				$ret = array_merge($ret, array(
					'status' => 'done',
					'msg' 	=> 'no searches found to be run at this moment - db step.' . $amz_verify_msg,
				));
				return $ret;
			}
			$prods = $res;
			...
			foreach ($prods as $key => $row) {
				...
				$searchParams = $this->get_search_params( $row );
				...
				if ( !$search_stat ) { // INVALID
					...
					$pmsSetupHelper = array(
						$prefix.'country' 				=> $searchParams['country'],
						$prefix.'main_aff_id'			=> $searchParams['main_aff_id'],
					);
		...
		private function get_search_params( $row=array() ) {
			$ret = array(
				'provider' 		=> 'amazon',
				'country'		=> '',
				'main_aff_id'	=> '',
			...
			$country = isset($params_db['extra_params'], $params_db['extra_params']['country'])
				? $params_db['extra_params']['country'] : $this->settings[$prefix.'country'];

			$main_aff_id = isset($params_db['extra_params'], $params_db['extra_params']['main_aff_id'])
				? $params_db['extra_params']['main_aff_id'] : $this->settings[$prefix.'main_aff_id'];
		...
		private function get_product_info( $asin, $pms=array() ) {
