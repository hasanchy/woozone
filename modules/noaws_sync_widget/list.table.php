...
class WooZoneNoAwsSyncWidgetList extends WP_List_Table {
	...
	public function get_syncs( $per_page = 5, $page_number = 1 ) {
		...
		if ( ! empty( $_REQUEST['orderby'] ) ) {
			$allowed_orderby = array('ID', 'bulk_asins', 'action', 'status', 'status_msg', 'country', 'widget_response_date', 'created_date');
			$orderby = in_array( $_REQUEST['orderby'], $allowed_orderby ) ? $_REQUEST['orderby'] : 'ID';
			$sql .= ' ORDER BY ' . $orderby;
			$allowed_order = array('ASC', 'DESC');
			$order = ! empty( $_REQUEST['order'] ) && in_array( strtoupper($_REQUEST['order']), $allowed_order ) ? strtoupper($_REQUEST['order']) : 'ASC';
			$sql .= ' ' . $order;
		}
		else {
			$sql .= ' ORDER BY ID ASC';
	...
	protected function extra_tablenav( $which )
	{
		...
		$html[] = '</div>';
		$html[] = '<a target="_blank" href="' . ( admin_url('admin.php?page=WooZone_no_aws_keys_sync_widget&sub_action=add_all_bulks') ) . '" class="button button-primary button-medium">Regenerate chunks</a>';
