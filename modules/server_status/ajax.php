		...
		*/
		public function ajax_request()
		{
			...
			if( in_array( 'remote_get', array_values($actions)) ){
				...
				$response = wp_remote_get('http://webservices.amazon.' . $this->the_plugin->amz_settings['country']);
				
				if ( ! is_wp_error( $response ) && $response['response']['code'] >= 200 && $response['response']['code'] <= 405 ) {
