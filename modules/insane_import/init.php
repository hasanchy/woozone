...
if (class_exists('WooZoneInsaneImport') != true) {
	class WooZoneInsaneImport
	{
		...
		private static $optionalParameters = array(
			'amazon'	=> array(
				'Actor'             => 'input', //added on 2020-march-26
				'Artist'            => 'input', //added on 2020-march-26
				'Author' 			=> 'input', //added on 2020-march-26
				'BrowseNode'        => 'select',
				'Brand'             => 'input',
				'Condition'         => 'select',
				'DeliveryFlags' 	=> 'select', //added on 2020-march-26
				...
				'Title' 			=> 'input', //added on 2020-march-26

				'Sort'              => 'select',

				//'Availability' 		=> 'select', //NOT NEEDED
				//'CurrencyOfPreference' => 'select', //MAYBE???
				//'LanguagesOfPreference' => 'select', //NOT NEEDED
			),
			'ebay'		=> array(
				'BrowseNode'			=> 'select',
		...
		public function __construct( $is_cron=false )
		{
			...
			add_action('wp_ajax_WooZoneIM_AfterImportingLastProduct', array( $this, 'after_importing_last_product' ), 10, 2);
...
<div class="WooZone-insane-container WooZone-big-buttons WooZone-insane-tabs" id="WooZone-wrap-loadproducts">
							...
							<div class="WooZone-insane-panel-headline WooZone-top-headline">
								...
								<a href="#WooZone-content-amazon" data-provider="amazon" class="on"><img src="<?php echo $this->module_folder;?>images/amz-logo.png" /></a>
								<?php } ?>
								<?php if ($this->the_plugin->providers_is_enabled('ebay')) { ?>
								<a href="#WooZone-content-ebay" data-provider='ebay'><img src="<?php echo $this->module_folder;?>images/ebay-logo.png" /></a>
								<?php } ?>
							</div>
...
<div id="WooZone-content-amazon" class="WooZone-insane-tab-content">
	...
	?>
									<div class="WooZone-insane-container WooZone-insane-tabs">
										<?php /*<div class="WooZone-insane-buton-logs" data-logcontainer="WooZone-logs-load-products"><?php _e('View Messages Log', $this->the_plugin->localizationName); ?></div>*/ ?>
										...
										<div class="WooZone-insane-panel-headline WooZone-operation-message">
											<span class="search_stats"></span>
											<a href="#WooZone-content-search-amazon" class="on"><?php _e('SEARCH FOR PRODUCTS', $this->the_plugin->localizationName);?></a>
											<a href="#WooZone-content-grab-amazon"><?php _e('GRAB PRODUCTS', $this->the_plugin->localizationName);?></a>
											<a href="#WooZone-content-bulk-amazon"><?php _e('ALREADY HAVE A LIST?', $this->the_plugin->localizationName);?></a>
											<a href="#WooZone-content-extension"><?php _e('Chrome Extension', $this->the_plugin->localizationName);?></a>
										</div>
										<div class="WooZone-insane-tabs-content">
											<div class="WooZone-content-scroll">
												
												<div id="WooZone-content-search-amazon" class="WooZone-insane-tab-content">
													<!-- Search buttons -->
													<div class="WooZone-insane-tab-search-buttons-container">
														<form class="WooZone-search-products">
															<ul class="WooZone-insane-tab-search-buttons">
																<li>		            						
																	<span class="tooltip" title="Choose Keyword"><i class="fa fa-search"></i></span>
																	<input type="text" id="WooZone-search-keyword" name="WooZone-search[keyword]" placeholder="<?php _e('Keyword', $this->the_plugin->localizationName);?>" autocomplete="off" value="<?php echo isset($_REQUEST['keyword']) ? esc_attr($_REQUEST['keyword']) : '';?>">
																	<ul class="WooZone-search-completion"></ul>
																</li>
																<li class="WooZone-select-on-category">
																	<span class="tooltip" title="Choose Category or Custom BrowseNode"><i class="fa fa-sitemap"></i></span>
																	<select class="WooZone-search-category" name="WooZone-search[category]">
																...
																<li>
																	...
																	<select class="WooZone-search-nbpages" name="WooZone-search[nbpages]">
																...
																<?php
																$getAmazonSearchParams = $this->get_category_params_html('return', array(
																	'what_params'           => 'all',
																	'category'              => 'AllCategories',
																	'nodeid'                => 0,
																	'provider'              => 'amazon',
																));
																//var_dump('<pre>', $getAmazonSearchParams , '</pre>'); echo __FILE__ . ":" . __LINE__;die . PHP_EOL;
																if ( 'valid' == $getAmazonSearchParams['status'] ) {
																	echo $getAmazonSearchParams['html'];
																}
																?>
												...
												<div id="WooZone-content-grab-amazon" class="WooZone-insane-tab-content">
													<!-- Grab from amazon -->
													<form class="WooZone-grab-products">
														<label>
														...
														<input type="button" value="<?php _e('GET ASIN codes', $this->the_plugin->localizationName);?>" class="WooZone-button orange WooZone-grabb-button">
													</form>
												</div>
			
												<div id="WooZone-content-bulk-amazon" class="WooZone-insane-tab-content">
													<!-- ASINs Bulk Import -->
													<form class="WooZone-import-products WooZone-bulk-products">
														<h3><?php _e('ASIN codes', $this->the_plugin->localizationName);?>:</h3>
														<textarea id="WooZone-content-bulk-asin" class="WooZone-content-bulk-asin"></textarea>
														...
														<div class="WooZone-delimiters">
															...
															<input class="WooZone-addASINtoQueue" type="button" value="<?php _e('Add ASIN codes to Queue', $this->the_plugin->localizationName);?>" />
														</div>
													</form>	
												</div>
												...
												<?php /*<div id="WooZone-loadprods-status"></div>*/ ?>
...
<div id="WooZone-content-ebay" class="WooZone-insane-tab-content">
	...
	?>
						<div class="WooZone-insane-container WooZone-insane-tabs">
							...
							<div class="WooZone-insane-panel-headline WooZone-operation-message">
								<span class="search_stats"></span>
								<a href="#WooZone-content-search-ebay" class="on"><?php _e('SEARCH FOR PRODUCTS', $this->the_plugin->localizationName);?></a>
								<a href="#WooZone-content-bulk-ebay"><?php _e('ALREADY HAVE A LIST?', $this->the_plugin->localizationName);?></a>
							</div>
							<div class="WooZone-insane-tabs-content">
								<div class="WooZone-content-scroll">
									
									<div id="WooZone-content-search-ebay" class="WooZone-insane-tab-content">
										<!-- Search buttons -->
										<div class="WooZone-insane-tab-search-buttons-container">
											<form class="WooZone-search-products">
												<ul class="WooZone-insane-tab-search-buttons">
													...
													<li>
														...
														<span class="WooZone-choose-between"></span>
													</li>
													<li>
														...
														<input type="text" name="WooZone-search[page]" placeholder="<?php _e('Page nb', $this->the_plugin->localizationName);?>">
													</li>
													<?php
									...
									<div id="WooZone-content-bulk-ebay" class="WooZone-insane-tab-content">
										<!-- ASINs Bulk Import -->
										<form class="WooZone-import-products WooZone-bulk-products">
											<h3><?php _e('ASIN codes', $this->the_plugin->localizationName);?>:</h3>
											<textarea class="WooZone-content-bulk-asin"></textarea>
											<div class="WooZone-delimiters">
												<span><?php _e('ASIN delimiter by', $this->the_plugin->localizationName);?>:</span>
												<input type="radio" val="newline" name="WooZone-csv-delimiter" checked="" class="WooZone-csv-radio" id="WooZone-csv-radio-newline"><label for="WooZone-csv-radio-newline"><?php _e('New line', $this->the_plugin->localizationName);?> <code>\n</code></label>
												<input type="radio" val="comma" name="WooZone-csv-delimiter" id="WooZone-csv-radio-comma"><label for="WooZone-csv-radio-comma"><?php _e('Comma', $this->the_plugin->localizationName);?> <code>,</code></label>
												<input type="radio" val="tab" name="WooZone-csv-delimiter" id="WooZone-csv-radio-tab"><label for="WooZone-csv-radio-tab"><?php _e('TAB', $this->the_plugin->localizationName);?> <code>TAB</code></label>
											</div>
											<div class="WooZone-delimiters">
												<input class="WooZone-addASINtoQueue" type="button" value="<?php _e('Add ASIN codes to Queue', $this->the_plugin->localizationName);?>" />
											</div>
...
<?php 
		...
		public function loadprods_queue_by_asin( $retType='die', $pms=array() ) {
			...
			if ( !empty($asins) ) {
				...
				try {
					...
					$__msg = WooZoneGetExceptionMsg( $e );
		...
		public function loadprods_queue_by_search( $retType='die', $pms=array() ) {
			...
			// [FIX 09.08.2022] - DeliveryFlags must be array (https://webservices.amazon.com/paapi5/documentation/search-items.html)
			if( $_optionalParameters['DeliveryFlags'] ) {
				$_optionalParameters['DeliveryFlags'] = array($requestData['DeliveryFlags']);
			}
		...
		public function import_product( $retType='die', $pms=array() ) {
			...
			//die( var_dump( "<pre>", $args_add , "<pre>" ) . PHP_EOL .  __FILE__ . ":" . __LINE__  );  
		...
		public function loadprods_build_html( $prods=array(), $provider='amazon' ) {
			...
			foreach ($prods as $asin => $prod) {
				...
				if( isset($prod['Brand']) && trim($prod['Brand']) != '' ) {
				...
				if( $provider == 'ebay' && isset($prod['__extra']['Seller']['UserID']) ) {
					$html[] = 		'<div class="WZC-keyword-attached-brand">'.__('by:', $this->the_plugin->localizationName).' <span class="tooltip" title="' . $prod['__extra']['Seller']['UserID'] . '">'. $prod['__extra']['Seller']['UserID'] .'</span></div>';
				}
				$html[] = 	'</div>';
				$html[] = 	'<div class="WooZone-provider-line WooZone-provider-'.$provider.'">'.strtolower($provider).'</div>';
				$html[] = '</li>';
			}
			return implode('', $html);
		}
		...
		public function build_searchform_element( $elm_type, $param, $value, $default, $extra=array() ) {
			...
			if ( $elm_type == 'input' ) {
				...
				if ( !empty($desc) && is_array($desc) ) {
					$css = ' class="WooZone-search-opt-desc"';
				}
		...
		public function get_category_params_html( $retType='die', $pms=array() ) {
			...
			if (
				!empty($optionalParameters)
				&& in_array( $provider, array('amazon', 'alibaba') )
			) {
				if (
					!empty($nodeid)
					|| ( 'amazon' === $provider && in_array($this->the_plugin->amzapi, array('newapi', 'creatorsapi')) )
				) {
					$ItemSearchParameters = $this->get_ws_object( $provider )->getAmazonItemSearchParameters();
				}
			}
			//var_dump('<pre>', $ItemSearchParameters , '</pre>'); echo __FILE__ . ":" . __LINE__;die . PHP_EOL;
			...
			//var_dump('<pre>', $ItemSortValues , '</pre>'); echo __FILE__ . ":" . __LINE__;die . PHP_EOL;
			...
			foreach ($optionalParameters as $oparam => $type) {
				...
				if (
					(
						!isset($ItemSearchParameters[$category])
						|| !in_array($oparam, $ItemSearchParameters[$category])
					)
				...
				$desc           = array();
				$global_desc    = isset($WooZone_search_params_desc["$provider"]["$oparam"])
					? $WooZone_search_params_desc["$provider"]["$oparam"] : '';
				$value          = isset($WooZone_search_params["$provider"]["$oparam"])
					? $WooZone_search_params["$provider"]["$oparam"] : '';
				...
				// amazon | alibaba
					...
					foreach ( $value as $skey => $stext ){
						...
						$desc["$skey"] = $WooZone_search_params_sort["$provider"]["$skey"];
					}
				}
				// ebay
					...
					$value = (array) $value;
					foreach ( $value as $skey => $stext ){
						$desc["$skey"] = isset($WooZone_search_params_sel["$provider"]["$oparam"]["$skey"]) 
							? $WooZone_search_params_sel["$provider"]["$oparam"]["$skey"] : $global_desc;
					}
				}
