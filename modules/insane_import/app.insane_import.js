...
WooZoneInsaneMode = (function($) {
		...
		$(document).ready(function() {
			...
			containers = {
				loadprods:      {
					...
					mainwrap     : maincontainer.find('#WooZone-wrap-loadproducts'),
			...
			for (var pi in providers) {
				...
				for (var ppi in pv.loadprods) {
					var ppv = pv.loadprods[ppi],
						ppel = '#WooZone-content-{el}-{provider}'.replace('{el}', ppv).replace('{provider}', pi);
				...
				containers.loadprods[pi]['bigwrap'] = wrap ? wrap.parents('.WooZone-insane-container:first') : null;
 
				containers.loadprods[pi]['menu'] = containers.loadprods[pi]['bigwrap']
					? containers.loadprods[pi]['bigwrap'].find('.WooZone-insane-panel-headline ') : null;
			}
	...
	var queueprod = (function() {
	    ...
	    function clear_queue() {
	        // empty the list before search
            containers.queueprods.prods.html('<ul class="WZC-keyword-attached WooZone-insane-bigscroll"></ul>');
	    }
	...
	function triggers()
	{
		...
		maincontainer.on('click', ".WooZone-insane-buton-logs", function(e){
			...
			//}
			wrapp.toggle('WooZone-logs-open');
	...
	var loadprod = (function() {
		...
		function triggers( provider ) {
			...
			if ( 'amazon' == provider ) {
				...
				// BULK LOAD ASINs
				box.on('click', 'form.WooZone-bulk-products .WooZone-addASINtoQueue', function(e) {
					e.preventDefault();
				...
				//var categ_wrap = search_wrap.find('.WooZone-select-on-category');
			...
			if ( 'envato' == provider ) {
				...
				box.on('click', ".WooZone-envato-sites-selected a", function(e){
					e.preventDefault();
				});
				
				box.on('click', ".WooZone-envato-sites a", function(e){
					e.preventDefault();
					
					var clone = $(this).clone();
					maincontainer.find(".WooZone-envato-sites-selected").html( clone );
					maincontainer.find(".WooZone-envato-sites-container input").val( clone.attr("href").replace("#", "" ) );
				});
				
				// fix overflow for ul sites!
				box.find('__.WooZone-envato-sites-container').hover(function() {
					box.find('.WooZone-insane-tab-search-buttons').addClass('WooZone-envato-sites-on');
				}, function() {
					box.find('.WooZone-insane-tab-search-buttons').removeClass('WooZone-envato-sites-on');
				});
			}
			if ( 'ebay' == provider ) {
				...
				// search products - sort parameter tooltip
				search_wrap.on('change', 'select.WooZone-search-opt-desc', function (e) {
					var that    = $(this);
					sort_tooltip( that );
				});
			}
		...
		function bulk_add_asin_to_queue( form, pms ) {
			...
			var box         = containers.loadprods[provider].bulk,
				form        = typeof form != 'undefined' ? form : box.find('form.WooZone-bulk-products'),
				asins_str   = $.trim( form.find('.WooZone-content-bulk-asin').val() ),
		...
		function grab_parse_url( form, pms ) {
			...
			var box         = containers.loadprods[provider].grab,
				form        = typeof form != 'undefined' ? form : box.find('form.WooZone-grab-products');
			...
			data['nonce'] = WooZoneHelper.nonce;
		...
		function search_prods( form, pms ) {
			...
			var box             = containers.loadprods[provider].search,
				...
				nodeid          = null;
			var //search_wrap     = containers.loadprods[provider].search,
				category_dropdown = form.find('select.WooZone-search-category'),
				category_id     = category_dropdown.length
					? category_dropdown.find('option:selected').data('nodeid') : 0,
				search_type_    = form.find('input[name="WooZone-search[search_type]"]'),
			...
			data.push(
				...
				{name:'nonce',  value:WooZoneHelper.nonce}
			);
			...
			if ( $.isArray(form_params) ) {
				...
				if ( category_id ) {
					form_params.push(
						{name: 'WooZone-search[category_id]', value: category_id}
					);
				}
				if ( search_type ) {
					form_params.push(
						{name: 'WooZone-search[search_type]', value: search_type}
					);
				}
			...
			data.push({'nonce': WooZoneHelper.nonce});
		...
		function export_asins( form ) {
			...
			var box = containers.queueprods.export,
				form = typeof form != 'undefined' ? form : box.find('form#WooZone-export-form'),
		...
		function get_browse_nodes( that, category, pms ) {
			...
			var box             = containers.loadprods[provider].search,
				...
				ascensor_value  = that.hasClass('WooZone-search-category')
	...
	var importprod = (function() {
		...
		function after_importing_last_product( callback ) {
			...
			data.push(
				{name: 'debug_level',       value: debug_level},
				{name:'nonce',  value:WooZoneHelper.nonce},
				{name: 'action',            value: 'WooZoneIM_AfterImportingLastProduct'}
			);
	...
	{   
		...
		params['nonce'] = WooZoneHelper.nonce;
