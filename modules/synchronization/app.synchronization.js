...
WooZoneTailSyncMonitor = (function ($) {
	...
	function triggers() {
		...
		var make_request = function( action, params, callback ){
			...
			if ( misc.hasOwnProperty(params, 'filterby_provider') ) {
				filterby_provider = params['filterby_provider'];
			}
			...
			else if ( 'filterby_provider' == action ) {
				data.push({name: 'filterby_provider', value: filterby_provider});
			}
		...
		//filterby_provider
		mainsync.find( settings_wrapp ).on('change', 'select[name=filterby_provider]', function(e){
			e.preventDefault();

			make_request( 'filterby_provider', {
				'filterby_provider' : $(this).val()
			} );
		});
